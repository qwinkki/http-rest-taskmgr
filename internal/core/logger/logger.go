...
func NewLogger(config Config) (*Logger, error) {
	...
	core := zapcore.NewTee(
		zapcore.NewCore(encoder, zapcore.AddSync(os.Stdout), zapLvl),
		zapcore.NewCore(encoder, zapcore.AddSync(logFile), zapLvl),
	)
...
func (l *Logger) Close() {
	if err := l.file.Close(); err != nil {
		fmt.Println("failed to close applecation logger: ", err)
	}
}
